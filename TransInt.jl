...
function parse( expr::Array{Any} )
    ...
    elseif expr[1] == :+ && length(expr) > 3
        addArray = Any[]
        push!(addArray, parse(expr[2]))
        push!(addArray, parse(expr[3]))
        
        for x=4 : length(expr)
            push!(addArray, parse(expr[x]))
        end
        return AddNode(addArray)
    ...
    elseif expr[1] == :and
        if length(expr) < 3
            ...
            andArray = Any[]
            push!(andArray, parse(expr[2]))
            push!(andArray, parse(expr[3]))
            
            for x=4 : length(expr)
                push!(andArray, parse(expr[x]))
            end
            return AndNode(andArray)
        end
...
function analyze( ast::OneopNode )
    lhs = analyze(ast.lhs)
    return OneopNode(ast.op, lhs)
end

function analyze( ast::If0Node )
    cond = analyze(ast.cond)
    zerobranch = analyze(ast.zerobranch)
    nzerobranch = analyze(ast.nzerobranch)
    return If0Node(cond, zerobranch, nzerobranch)
end

function analyze( ast::AddNode )
    if length(ast.nums) == 2
        return BinopNode(+, analyze(ast.nums[1]), analyze(ast.nums[2]))
    else
        return BinopNode(+, analyze(ast.nums[1]), analyze(AddNode(ast.nums[2:end])))
    end
end
...
function analyze( ast::WithNode )
    ...
    symArray = Symbol[]
    exprArray = Any[]
    for x in ast.bindings
        push!(symArray, x.sym)
        push!(exprArray, analyze(x.binding_expr))
    end
    fdn = FuncDefNode( symArray, analyze(ast.body) )
    
    return FuncAppNode( fdn, exprArray )
end

function analyze( ast::AndNode )
    if length(ast.extra) == 1
        return If0Node(analyze(ast.extra[1]), NumNode(0), NumNode(1))
    else
        return If0Node(analyze(ast.extra[1]), NumNode(0), analyze(AndNode(ast.extra[2:end])))
    end
end
...
function calc( ast::BinopNode, env::Environment )
    ...
    if typeof(rhs) != NumVal || typeof(lhs) != NumVal
        throw(LispError("Cannot operate on non numbers!"))
        
    elseif rhs.n == 0 && ast.op == / 
