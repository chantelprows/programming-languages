...
function type_of_expr(ast::IsZeroNode, env::TypeEnvironment)
    ...
    throw(LispError("Argument needs to be a number"))
end

function type_of_expr(ast::IfBNode, env::TypeEnvironment)
    if type_of_expr(ast.cond, env) != BoolType()
        throw(LispError("Condition needs to be boolean"))
    end
    
    zBranch = type_of_expr(ast.zerobranch)
    nBranch = type_of_expr(ast.nzerobranch)
    
    if !same_type(zBranch, nBranch)
        throw(LispError("Branch types need to match"))
    end
    return nBranch
end

function type_of_expr(ast::WithNode, env::TypeEnvironment)
    beType = type_of_expr(ast.binding_expr)
    newEnv = ExtendedTypeEnv(ast.sym, beType, env)
    bodyType = type_of_expr(ast.body, newEnv)
...
function type_of_expr(ast::VarRefNode, env::TypeEnvironment)
    if env == EmptyTypeEnv()
        throw(LispError("Symbol not found"))
...
function type_of_expr(ast::FuncDefNode, env::TypeEnvironment)
    ...
    return FuncType(ast.formal_type, bodyType)
end

function type_of_expr(ast::FuncAppNode, env::TypeEnvironment)
    funcType = type_of_expr(ast.fun_expr)
    argType = type_of_expr(ast.arg_expr)
    
    if argType != funcType.arg_type
        throw(LispError("Types don't match"))
    end
...
function type_of_expr(ast::NIsEmptyNode, env::TypeEnvironment)
    if type_of_expr(ast.list, env) != NListType()
        throw(LispError("must be a list"))
    end
...
function type_of_expr(ast::NConsNode, env::TypeEnvironment)
    if type_of_expr(ast.f) != NumType()
        throw(LispError("First must be number"))
        
    elseif type_of_expr(ast.r) != NListType()
        throw(LispError("Rest must be list"))
    end
